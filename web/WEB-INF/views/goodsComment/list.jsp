                ...
                <table class="table">
                    ...
                    <c:forEach items="${list}" var="e" varStatus="st">
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-primary" type="button" onclick="deleteRole('${e.id}')">删除
                                </button>
                            </td>
                        </tr>
                    </c:forEach>
...
<script type="text/javascript">
    function searchIndex(e) {
        ...
        if (_index <= ${page.pageCount} && _index >= 1) {
            window.location.href = "#module=/listGoodsComments&page=" + _index +
            "&size=" + size + "&_t="+ new Date().getTime()+"&is_del="+is_del ;
        } else {
    ...
    function nextPage(_page) {
        ...
        if ((page <= ${page.pageCount} && page >= 1) ) {
            window.location.href = "#module=/listGoodsComments&page=" + page + "&size=" + size + "&_t="+ new Date().getTime()
            +"&is_del="+is_del;
        } else {
            alert("请输入1-${page.pageCount}的页码数");
        }
    ...
    function Daochu_Select() {
        ...
        if (select_id == '') {
            ...
            if (confirm("确定要导出所选择的数据吗？")) {
                $.ajax({
                    url: "/goodsCommentsExportExcel.do",
                    data: {"ids": select_id},
                    type: "POST",
